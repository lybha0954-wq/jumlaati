    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-6 md:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-1">
          <h1 className="text-3xl font-black bg-gradient-to-r from-emerald-400 to-teal-400 bg-clip-text text-transparent">الملف الشخصي والإعدادات</h1>
          <p className="text-sm text-slate-400">أدر بيانات حسابك وتفضيلاتك وتواصل مع الدعم</p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 flex flex-col md:flex-row items-center gap-6">
          <div className="h-24 w-24 rounded-3xl bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border border-white/10 flex items-center justify-center text-emerald-400">
            <User size={48} />
          </div>
          <div className="flex-1 text-center md:text-left">
            <h2 className="text-2xl font-black text-white">{profile?.full_name || 'اسم المستخدم'}</h2>
        ...
        <div className="flex gap-2">
          <button 
            ...
            className={`flex-1 py-3 rounded-2xl text-sm font-bold transition ${activeTab === 'account' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10'}`}
          >
            معلومات الحساب
          </button>
          <button 
            onClick={() => setActiveTab('settings')}
            className={`flex-1 py-3 rounded-2xl text-sm font-bold transition ${activeTab === 'settings' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10'}`}
          >
            الإعدادات
          </button>
          <button 
            onClick={() => setActiveTab('support')}
            className={`flex-1 py-3 rounded-2xl text-sm font-bold transition ${activeTab === 'support' ? 'bg-gradient-to-r from-emerald-600 to-teal-600 text-white shadow-lg shadow-emerald-500/20' : 'bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10'}`}
          >
            الدعم الفني
          </button>
        </div>
          ...
          {activeTab === 'account' && (
            <div className="space-y-6">
              <h3 className="text-xl font-bold text-white flex items-center gap-2">
                <User size={20} className="text-emerald-400" /> بيانات الحساب
              </h3>
              ...
              <div className="flex items-center justify-between bg-slate-900/60 border border-slate-700 rounded-2xl p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">استلام إشعار عند تغير حالة الطلب</p>
                  </div>
                </div>
