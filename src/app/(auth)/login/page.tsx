...
import { AlertCircle, ArrowRight, Building2, KeyRound, Info, Lock, Mail, Phone, ShieldCheck, Store } from 'lucide-react';
      ...
      <div className="grid w-full max-w-6xl gap-8 xl:grid-cols-[420px_minmax(420px,1fr)]">
        ...
        <section className="rounded-[32px] border border-white/10 bg-slate-900/95 p-8 shadow-2xl backdrop-blur-xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4 text-slate-300 text-sm font-arabic">
              ...
              <div className="mt-4 grid gap-3">
                {(['retailer', 'supplier', 'admin'] as AuthRole[]).map((demoRole) => {
                  const demo = DEMO_CREDENTIALS[demoRole] as { email?: string; phone?: string; password?: string; accessKey?: string };
                  return (
                    <button
                      key={demoRole}
                      type="button"
                      onClick={() => handleDemoFill(demoRole)}
                      className="rounded-3xl border border-white/10 bg-slate-950/90 px-4 py-3 text-left text-sm text-slate-200 transition hover:border-emerald-500/80 hover:bg-slate-900"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          <p className="font-semibold font-arabic">{roles[demoRole].label}</p>
                          <p className="mt-1 text-xs text-slate-400 font-arabic">
                            {demoRole === 'admin'
                              ? `مفتاح: ${demo.accessKey}`
                              : `ايميل: ${demo.email} | هاتف: ${demo.phone}`}
                          </p>
                        </div>
                        <ArrowRight size={18} />
                      </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-950/50 p-4 text-xs text-slate-500 font-arabic leading-6">
              <p>بحسب اللوائح العراقية، يجب أن يكون تسجيل الدخول آمناً وأن لا يشارك المستخدم بيانات حسابه الخاص مع الغير.</p>
              <p className="mt-2">إنها بيئة عرض تجريبي. سيتم تحويل رقم الهاتف الوطني إلى البريد التجريبي عند عدم وجود إعداد Supabase جاهز.</p>
            </div>
          </form>
        </section>
      </div>
