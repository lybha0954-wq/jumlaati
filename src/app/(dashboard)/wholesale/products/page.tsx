...
export default function WholesaleProductsPage() {
  ...
  const columns = [
    ...
    { key: "stock", header: "الكمية", render: (row: any) => (
        row.stock < 10 ? <Badge variant="destructive">منخفض ({row.stock})</Badge> : <Badge>{row.stock}</Badge>
    )},
    { key: "actions", header: "إجراءات", render: (row: any) => (
        <div className="flex gap-2">
            <Button size="sm" variant="outline" onClick={() => { setEditingProduct(row); setShowForm(true); }}>
                <Pencil size={14} />
            </Button>
