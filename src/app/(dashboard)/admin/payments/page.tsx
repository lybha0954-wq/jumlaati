...
export default function AdminPaymentsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Topbar />
      <div className="p-6">
        <h1 className="text-3xl font-bold mb-6 flex items-center gap-2"><CreditCard className="text-primary" size={28} /> سجل المدفوعات</h1>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          {payments.length === 0 ? (<div className="py-10 text-center text-gray-500">لا توجد مدفوعات حالياً.</div>) : (<DataTable data={payments} columns={columns} />)}
        </div>
      </div>
    </div>
  );
