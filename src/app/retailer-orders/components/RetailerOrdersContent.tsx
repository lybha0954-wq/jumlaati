              ...
              <div className="bg-card border border-border rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
                  ...
                  <div className="bg-muted/30 rounded-2xl p-4 border border-border">
                    <p className="text-xs font-semibold text-muted-foreground font-arabic mb-1.5 flex items-center gap-1.5">
                      <Building2 size={12} />
                      عنوان التوصيل (تلقائي من بيانات المحل)
                    </p>
                    <p className="text-sm font-bold text-foreground font-arabic">{RETAILER_PROFILE.storeName}</p>
                    <p className="text-xs text-muted-foreground font-arabic mt-0.5">{storeLocation}</p>
                  </div>

                  {/* Payment Method */}
                  <div>
                    <p className="text-sm font-bold text-foreground font-arabic mb-3">طريقة الدفع</p>
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={() => setPaymentMethod('cod')}
                        className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${paymentMethod === 'cod' ? 'border-primary bg-primary/5' : 'border-border bg-card hover:border-primary/40'}`}
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${paymentMethod === 'cod' ? 'bg-primary/10' : 'bg-muted'}`}>
                          <Wallet size={20} className={paymentMethod === 'cod' ? 'text-primary' : 'text-muted-foreground'} />
                        </div>
                        <span className="text-xs font-bold font-arabic text-foreground">الدفع عند الاستلام</span>
                        <span className="text-[10px] text-muted-foreground font-arabic">نقداً أو بطاقة</span>
                      </button>
                      <button
                        onClick={() => setPaymentMethod('credit')}
                        className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all ${paymentMethod === 'credit' ? 'border-primary bg-primary/5' : 'border-border bg-card hover:border-primary/40'}`}
                      >
                        <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${paymentMethod === 'credit' ? 'bg-primary/10' : 'bg-muted'}`}>
                          <CreditCard size={20} className={paymentMethod === 'credit' ? 'text-primary' : 'text-muted-foreground'} />
                        </div>
                        <span className="text-xs font-bold font-arabic text-foreground">الرصيد الائتماني</span>
                        <span className="text-[10px] text-emerald-600 font-arabic font-semibold tabular-nums">{fmt(creditAvailable)} متاح</span>
                      </button>
                    </div>
                  </div>

                  {/* Order Summary */}
                  <div className="bg-muted/20 rounded-2xl p-4 space-y-2 border border-border">
                    <div className="flex justify-between text-sm font-arabic">
